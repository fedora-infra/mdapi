...
var filedict = map[string]models.FileUnit{
	"primary": {
		Keep: true,
...
func WithMockedReadMetadata(t *testing.T, extn string) {
	...
	driver.ReadMetadata = func(unit *models.LinkUnit) ([]models.FileUnit, error) {
		rslt := []models.FileUnit{
			{
				Keep: true,
				Name: fmt.Sprintf("%s-primary.xml.%s", hash, extn),
				Type: "primary",
				Path: fmt.Sprintf("%s/%s-primary.xml.%s", host, hash, extn),
				Hash: models.Checksum{Type: "sha256", Data: hash},
			},
			{
				Keep: true,
...
func WithMockedDownloadRepositories(t *testing.T, iden string, extn string) {
	...
	driver.DownloadRepositories = func(flut *models.FileUnit, vers *string, stab int64, cast *int, loca *string) error {
...
func TestHandleRepositories_Failure_ReadMetadata(t *testing.T) {
	...
	original := driver.ReadMetadata
	driver.ReadMetadata = func(unit *models.LinkUnit) ([]models.FileUnit, error) {
		return []models.FileUnit{}, errors.New("ReadMetadata failed")
	}
	defer func() {
		driver.ReadMetadata = original
	}()
